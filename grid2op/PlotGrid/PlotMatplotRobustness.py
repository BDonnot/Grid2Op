...
class PlotMatplot(BasePlot):
    ...
    def draw_legend(self, figure, observation):
        ...
        legend_help = [
            ...
            patches.Circle((0.5, 0.5), self._sub_radius, edgecolor=self._sub_edge_color, facecolor='white', linewidth=1),
            patches.Circle((0.5, 0.5), self._load_radius, edgecolor=self._load_edge_color, facecolor='white', linewidth=1),
            patches.Circle((0.5, 0.5), self._gen_radius, edgecolor=self._gen_edge_color, facecolor='white', linewidth=1),
        ...
        self.ax.legend(legend_help, [
            ...
            handler_map={patches.Circle: HandlerEllipse()},
            # title=title_str
        )
        if self._colorbar_mappable is not None:
            cbar = plt.colorbar(ScalarMappable(cmap=self._colorbar_mappable), ax=self.ax, aspect=40)
            cbar.ax.set_ylabel('Probability of overflow after disconnection', rotation=270, labelpad=20)
...
class HandlerEllipse(HandlerPatch):
    def create_artists(self, legend, orig_handle,
        ...
        r = orig_handle.radius
        center = 0.5 * width - 0.5 * xdescent, 0.5 * height - 0.5 * ydescent
        p = patches.Ellipse(xy=center, width=0.04 * r * (width + xdescent),
                             height=0.12 * r * (height + ydescent))
